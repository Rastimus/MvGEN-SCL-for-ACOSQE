...
def get_orders(task, data, args, sents, labels):
    ...
    # tokenizer = T5Tokenizer.from_pretrained("/disc2/yanhong.wang/ABSA-QUAD-master/t5-base")
    # model = MyT5ForConditionalGenerationScore.from_pretrained(
    #     "/disc2/yanhong.wang/ABSA-QUAD-master/t5-base").to(device)
...
def order_scores_function(quad_list, cur_sent, model, tokenizer, device, task):
    ...
    all_orders_list = []
    # print("First item in quad_list:", list(quad_list[0].keys()))
    ...
    for each_order in all_orders:
        ...
        print("Generated order:", cur_order)
        all_orders_list.append(cur_order)
        cur_target = []
        for each_q in quad_list:
            if cur_order not in each_q:
                print("Available keys in dict:", list(each_q.keys()))
                print("Attempting to access key:", cur_order)
...
def choose_best_order_global(sents, labels, model, tokenizer, device, task):
    ...
    for i in range(len(sents)):
        ...
        for _tuple in label:
            ...
            # print("element_list before loop:", element_list)
            # print("q:", q)
            # # 循环后
            # print("element_list after loop:", element_list)
...
def add_prompt(sent, orders, task, data_name, args):
    sufix_sentence = "What are the"
    lastfix_sentence = "in :"
    prefix_after = sufix_sentence.split()
    lastfix_after = lastfix_sentence.split()
    if args.multi_task:
    ...
    if args.ctrl_token == "none":
        ...
        # sent =  prefix_after + orders + lastfix_after + sent
        sent = sent + orders
...
class ABSADataset(Dataset):
    ...
    def get_raw_labels(self):
        if self.args.multi_task:
            results = get_transformed_io_unified(self.data_path, self.task_name,
                                               self.data_name, self.data_type,
                                               self.top_k, self.args)
        else:
            results = get_transformed_io(self.data_path, self.data_name,
                                       self.data_type, self.top_k, self.args)
    ...
    def _build_examples(self):
        ...
        if self.args.multi_task:
            ...
            inputs, targets, labels = get_transformed_io(self.data_path,
                                                 self.data_name,
...
class MvPSCLDataset(ABSADataset):

    def __getitem__(self, index):
        original_index = index // self.top_k
        source_ids = self.inputs[index]["input_ids"].squeeze()
        target_ids = self.targets[index]["input_ids"].squeeze()
        ...
        sentiment_label = torch.tensor(self.contrastive_labels['sentiment'][original_index])
        aspect_label = torch.tensor(self.contrastive_labels['aspect'][original_index])
        opinion_label = torch.tensor(self.contrastive_labels['opinion'][original_index])
    ...
    def _build_examples(self):
        inputs, targets, labels = get_transformed_io(self.data_path, self.data_name,
                                       self.data_type, self.top_k, self.args)
        ...
        def get_sentiment_labels(labels_in, task):
            sentiment_dict = {
                'bad': 0,
                'ok': 1,
                'great': 2,
            ...
            # 处理每个句子
            for ex in labels_in:
                sample_labels = set()  # 使用集合来存储独特的情感标签
                
                # 处理句子中的每个四元组
                for quad in ex:
                    at, ac, sp, ot = get_task_tuple(quad, task)
                    if sp:  # 如果存在情感极性
                        label = sentiment_dict[sp.lower()]
                        assert label in [0, 1, 2, 3]
                        sample_labels.add(label)
                
                # 确定最终的句子标签
                if len(sample_labels) == 1:
                    final_label = sample_labels.pop()  # 只有一种情感
                else:
                    final_label = 3  # mixed，多种情感
            ...
            print("Finished building examples")
            print(f"Number of calculated sentiment labels: {len(sentiment_labels)}")
    
            return sentiment_labels

        def get_opinion_labels(labels_in, task):
            opinion_dict = {
                'NULL': 0,
                'EXPLICIT': 1,
                'BOTH': 2,
            ...
            for ex in labels_in:
                ...
                for quad in ex:
                    at, ac, sp, ot = get_task_tuple(quad, task)
                    # 如果有具体的意见词，说明是 EXPLICIT
                    if ot and ot.lower() == 'null':
                        opinions.add('NULL')
                    elif ot:
                        opinions.add('EXPLICIT')
                
                # 根据收集到的类型判断标签
                if 'NULL' not in opinions:
                    label = opinion_dict['EXPLICIT']
                else:
                    if len(opinions) == 1:  # 只有 NULL
                        label = opinion_dict['NULL']
                    else:  # 既有 NULL 又有 EXPLICIT
                        label = opinion_dict['BOTH']
                
                opinion_labels.append(label)
            from collections import Counter
            print("Opinion distribution")
            print(Counter(opinion_labels))
            
            return opinion_labels

        def get_aspect_labels(labels_in, task):
            aspect_dict = {
                'NULL': 0,
                'EXPLICIT': 1,
                'BOTH': 2,
            ...
            for ex in labels_in:
                ...
                for quad in ex:
                    at, ac, sp, ot = get_task_tuple(quad, task)
                    # 'it' 表示原来是 NULL aspect
                    if at == 'it':
                        aspects.add('NULL')
                    else:
                        aspects.add('EXPLICIT')
                
                # 根据收集到的类型判断标签
                if 'NULL' not in aspects:
                    label = aspect_dict['EXPLICIT']
                else:
                    if len(aspects) == 1:  # 只有NULL
                        label = aspect_dict['NULL']
                    else:  # 既有NULL（'it'）又有EXPLICIT
                        label = aspect_dict['BOTH']
                
                aspect_labels.append(label)
            from collections import Counter
            print("Aspect distribution")
            print(Counter(aspect_labels))
            
            return aspect_labels
        self.contrastive_labels['sentiment'] = get_sentiment_labels(labels, self.task_name)
        self.contrastive_labels['opinion'] = get_opinion_labels(labels, self.task_name)
        self.contrastive_labels['aspect'] = get_aspect_labels(labels, self.task_name)
